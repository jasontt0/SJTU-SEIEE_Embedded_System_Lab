...
char time_record[256];
char RxBuf[256];            //定义数据缓冲区存放字符串
...
void UARTStringPut(uint8_t *cMessage)        //阻塞式发送字符
{
	while(*cMessage!='\0')
...
void UARTStringPutNonBlocking(const char *cMessage)     //非阻塞式发送字符
{
	while(*cMessage!='\0')
	{
...
void UARTStringGetNonBlocking(void)                    //非阻塞式获取字符
{
	 uint8_t cnt = 0;
...
{
	int32_t uart0_int_status;
	int8_t minute1,second1,minute2,second2;
  uart0_int_status 		= UARTIntStatus(UART0_BASE, true);		// Get the interrrupt status.

  UARTIntClear(UART0_BASE, uart0_int_status);								//Clear the asserted interrupts
  
	UARTStringGetNonBlocking();        //获取指令
	
	
	//执行指令
  if(RxBuf[2]==':'&&RxBuf[8]==':'&&RxBuf[5]=='+')            //不区分大小写
	{
			minute1=(RxBuf[0]-'0')*10+(RxBuf[1]-'0');
			second1=(RxBuf[3]-'0')*10+(RxBuf[4]-'0');
			minute2=(RxBuf[6]-'0')*10+(RxBuf[7]-'0');
			second2=(RxBuf[9]-'0')*10+(RxBuf[10]-'0');
			
			if(minute1>60||second1>60||minute2>60||second2>60)
			...
			else
			{
				...
				sprintf(time_record,"%02u%s%02u",minute,":",second);    //记录当前时间
				UARTStringPutNonBlocking(time_record);
			}
			memset(RxBuf, 0, sizeof(RxBuf));      //初始化
	}
	else
	{
		UARTStringPutNonBlocking("Invalid Inputs!!!");
