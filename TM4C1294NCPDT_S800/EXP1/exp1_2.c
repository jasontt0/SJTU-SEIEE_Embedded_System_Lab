...
void  PF0Flash(uint8_t,uint8_t);      // 根据传入的按键值，决定PF0快闪或慢闪
...
int main(void)
{
    ...
    while(1)                // 无限循环
    {
        ...
        PF0Flash(ui8KeyValue_0,ui8KeyValue_1);          // 根据传入的按键参数，决定PF0快闪或慢闪
    }
}
...
//
//**************************************************************************************
void PF0Flash(uint8_t ui8KeyValue_0,uint8_t ui8KeyValue_1)
{
    uint32_t ui32DelayTime;
	
    if (ui8KeyValue_0	== 0)                                  // PUSH1(USR_SW1-PJ0) 按下
		...
		if (ui8KeyValue_1	== 0)                                  // PUSH1(USR_SW1-PJ0) 按下
        GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_4, GPIO_PIN_4); // 点亮 LED4(D4-PF0)
    else                                                   // PUSH1(USR_SW1-PJ0) 松开
